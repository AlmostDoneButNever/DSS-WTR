...
from dss.models import (Product)
...
def questions_rsp():
    ...
    for obj in product:
        ...
        for key, x in instance.attrs.items():
            result[key].append(x.value)    
    df = pd.DataFrame(result)
    samplefood=df['ProductName'].tolist()
    ...
    materialselected = ['foodwaste','manure','woodwaste']
    return render_template('questions_template_rsp.html', materialselected = materialselected, samplefood = samplefood, samplefoodlen=len(samplefood))